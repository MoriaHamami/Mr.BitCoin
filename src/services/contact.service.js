...
async function getContacts(filterBy = { txt: '' }) {
    ...
    if (filterBy && filterBy.txt) {
        // contacts = filter(filterBy.txt)
        const filter = filterBy.txt.toLocaleLowerCase()
        contacts = contacts.filter(contact => {
            return contact.name.toLocaleLowerCase().includes(filter) ||
                contact.phone.toLocaleLowerCase().includes(filter) ||
                contact.email.toLocaleLowerCase().includes(filter)
        })
    }
    return contacts
}
